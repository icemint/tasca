...
> **NOT LEGAL ADVICE — REVIEW BEFORE PUBLISHING.** This document is a drafting starting point generated from a template and the *mill-deterrent-pack* clause library. It has **not** been reviewed by a licensed attorney. The entity, governing-law, venue, address, contact, and date fields have been filled (see the record below); the document must still be reviewed by counsel licensed in Wyoming (and, given your California users, by California counsel) before it is published or relied upon. See `INTEGRATION-NOTES.md` for counsel-review items.

---

## Completed terms (record)
...
| Arbitration administrator | American Arbitration Association (AAA), Consumer Arbitration Rules (then-current) |
| Notice address | 30 N Gould St, Ste R, Sheridan, WY 82801 |
| Legal / privacy contacts | legal@tasca.dev / privacy@tasca.dev |
| Effective / last updated | June 1, 2026 |

> **Still open (Privacy Policy):** the website/docs tracking-technology list (Privacy §6) and the CCPA/CPRA disclosures (Privacy §11.1) remain to be reconciled against an actual audit before publishing.
...
Tasca is an agentic project-delivery platform on which human users and AI coding agents collaborate on a shared project board, with capability-aware routing of work to agents and to cloud AI models. The Service may include project and issue management, AI agent execution, source-control (e.g., GitHub) integration, and related features.

**Self-hosted deployments.** Tasca may also be made available as self-hostable software under a separate license (see the applicable `LICENSE` file). These Terms govern the **hosted** Service operated by Company at tasca.dev. Where you operate your own self-hosted instance, you — not Company — are the operator and controller of that instance and the data in it, and these Terms govern only your use of Company-operated services (such as this website, hosted offerings, and any APIs we operate).
...
(b) **Your credentials and API keys.** You are responsible for any third-party credentials, API keys (including Anthropic Console API keys), OAuth authorizations (e.g., GitHub, Google), and model-provider accounts you connect to the Service, and for complying with the terms of those third parties. You must use Anthropic Console API keys billed to your organization where the Service calls Anthropic models; subscription-based OAuth credentials must not be used in violation of the applicable provider's policies.
...
(d) **Rights and compliance.** You represent that you have all rights necessary to submit Customer Content and to authorize the Service to act on the connected repositories and systems, and that your use complies with applicable law and any third-party terms.
...
The Service integrates with third-party services (including, without limitation, source-control providers, identity providers, and AI model providers). Your use of those services is governed by their terms and privacy practices, not these Terms. Company is not responsible for third-party services and does not guarantee their availability, security, or output.
