  ...
  return (
    ...
    >
      <Panel
        id="kanban-left"
        minSize="20%"
        className="min-w-0 h-full overflow-hidden bg-surface"
      >
        <ProjectKanbanBoard />
      </Panel>
      ...
      {isRightPanelOpen && (
        <Separator
          id="kanban-separator"
          className="w-1 bg-surface-2 outline-none hover:bg-signal/50 transition-colors cursor-col-resize"
        />
      )}
      ...
      {isRightPanelOpen && (
        <Panel
          ...
          minSize="400px"
          maxSize="800px"
          className="min-w-0 h-full overflow-hidden bg-surface-2"
        >
          <ProjectRightSidebarContainer />
        </Panel>
      )}
...
function ProjectKanbanInner({ projectId }: { projectId: string }) {
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center h-full w-full">
        <p className="text-fg-3">{t('states.loading')}</p>
      </div>
    );
  }
  ...
  if (!project) {
    return (
      <div className="flex items-center justify-center h-full w-full">
        <p className="text-fg-3">{t('kanban.noProjectFound')}</p>
      </div>
    );
  }
