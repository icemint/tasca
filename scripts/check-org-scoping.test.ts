...
describe('org-scoping guard — it MUST fire on raw tenant SQL outside the scoped layer', () => {
  ...
  it('covers all twelve tenant tables', () => {
    expect([...TENANT_TABLES].sort()).toEqual(
      ['dispatch_job', 'task', 'routing_decision', 'pull_request', 'platform_connection', 'webhook_event', 'proposal', 'usage_event', 'org_vendor_credential', 'governance_audit_event', 'org_invite', 'project'].sort()
    );
  });
});
