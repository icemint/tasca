...
description: "Deploy Tasca Cloud on any server using Docker Compose"
---

This guide covers deploying Tasca Cloud on any Linux server using Docker Compose. This approach works with any cloud provider (AWS, DigitalOcean, Hetzner, etc.) or on-premises server.
...
```env .env.remote
# Required secrets
TASCA_REMOTE_JWT_SECRET=<your_generated_jwt_secret>
...
services:
  ...
  remote-server:
    ...
    environment:
      ...
      TASCA_REMOTE_JWT_SECRET: ${TASCA_REMOTE_JWT_SECRET}
...
2. You should see the Tasca Cloud login page
