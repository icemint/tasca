...
When running Tasca on a remote server (e.g., accessed via Cloudflare tunnel, ngrok, or as a systemctl service), you can configure VSCode-based editors to open projects via SSH instead of assuming localhost.
...
- Tasca runs on a remote server (VPS, cloud instance, etc.)
...
Set a prefix for auto-generated branch names. When you create a new workspace or task, Tasca automatically creates a git branch for your changes.
...
Specify where Tasca stores workspace data. Workspaces are created in a `.tasca-workspaces` subdirectory within this path.
...
Configure how Tasca alerts you about task progress and status changes.

- **Sound Effects** - Play audio notifications when tasks complete, need attention, or encounter errors. Useful when working with multiple tasks or when Tasca runs in a background tab.
- **Push Notifications** - Receive browser notifications even when Tasca isn't in focus. Requires browser permission when first enabled.
...
Enable or disable telemetry data collection to help improve Tasca.
