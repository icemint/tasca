---
title: "Tasca MCP Server"
description: "Configure the Tasca MCP server"
---

Tasca exposes a local MCP (Model Context Protocol) server, allowing you to manage organisations, projects, issues, workspaces, and repositories from external MCP clients like Claude Desktop, Raycast, or even coding agents running within Tasca itself.

<Note>
This page covers connecting **external MCP clients** to Tasca's MCP server. For configuring MCP servers **within** Tasca for your coding agents, see the [MCP Server Configuration](/integrations/mcp-server-configuration) guide.
</Note>

<Info>
Tasca's MCP server is **local-only** - it runs on your computer and can only be accessed by applications installed locally. It cannot be accessed via publicly accessible URLs.
</Info>
...
This works if you're adding the Tasca MCP server to any [supported coding agent](/supported-coding-agents) **within** Tasca.

1. In Tasca Settings, navigate to the "MCP Servers" page
2. In the "Popular servers" section, click on the Tasca card
3. Click the `Save Settings` button

<Frame>
<img src="/images/tasca-mcp-server-config.png" alt="MCP Servers configuration page showing how to add Tasca MCP server" />
</Frame>
...
```json
{
  "mcpServers": {
    "tasca": {
      "command": "npx",
      "args": ["-y", "tasca@latest", "--mcp"]
    }
  }
}
```

`--mcp` launches the local MCP stdio server. Any additional arguments after `--mcp` are passed through to the `tasca-mcp` binary.

## Available MCP Tools

The Tasca MCP server provides tools for managing organisations, projects, issues, workspaces, and task execution.
...
3. **Automatic Issue Generation**: Your MCP client will use the Tasca MCP server to automatically create structured issues in your project
...
Your MCP client will use the `create_issue` tool to break this down into individual issues and add them to your Tasca project automatically.
...
### Internal Coding Agents (Within Tasca)

A powerful workflow involves using coding agents within Tasca that are also connected to the Tasca MCP server:
...
4. **Automatic Population**: The agent uses the MCP server to populate individual issues directly in the Tasca interface
...
Raycast is a popular MCP client that can connect to Tasca's MCP server. Here's how to configure it:
...
<Frame>
<img src="/images/tasca-raycast-mcp-part-1.png" alt="Raycast MCP configuration - adding Tasca server" />
</Frame>

Configure the Tasca MCP server in Raycast by adding the server details.
</Tab>

<Tab title="Step 2: Supply Command">
<Frame>
<img src="/images/tasca-raycast-mcp-part-2.png" alt="Raycast MCP configuration - server successfully added" />
</Frame>

Once configured, you'll see the Tasca MCP server listed and ready to use in Raycast.
</Tab>
</Tabs>
