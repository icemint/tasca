...
| DESIGN-agent-detail-view | Agent profile (bindings, capability, performance) | ✅ | Real data + live Pause/Resume + **capability editor (tier/specialties/concurrency/ceiling) + per-agent credentials + editable identity + agent.md** shipped #354–#372; agent-page Current-work task actions (interrupt/reassign/force-reset) wired to the live task endpoints (#374); escalate/deploy/assign remain gated placeholders (escalate→EM is #327); current-task UUID display partially resolved (#325 partial) | ▶W4-S3/S5 |
